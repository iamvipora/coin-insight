...
function MarketStatistics({ data }) {
  ...
  return (
    <div className={`flex items-center gap-4 p-2 h-28 w-full rounded-lg ${bgStyling} ${textStyling}`}>
      <div className='flex flex-col items-center place-content-center h-full w-full'>
        <h2 className='text-xl font-bold'>${formattedNumber}</h2>
        <span className='text-md'>{data.statsName}</span>
      </div>
    </div>
  )
}
